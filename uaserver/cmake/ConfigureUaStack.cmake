...
if (DEFINED uastack_SOURCE_DIR)
    ...
    # Search for the SDK again in CMAKE_INSTALL_PREFIX if CMAKE_INSTALL_PREFIX changed
    if (_PREV_INSTALL_PREFIX AND NOT ("${_PREV_INSTALL_PREFIX}" STREQUAL "${CMAKE_INSTALL_PREFIX}"))
        set(_PROBE_AGAIN ON)
    endif ()
    ...
    if (UASDK_BASE_DIR AND NOT IS_DIRECTORY ${UASDK_BASE_DIR}/include/uastack)
        message(FATAL_ERROR "UASDK_BASE_DIR does not point to folder containing AnsiC SDK include headers")
    elseif (NOT UASDK_BASE_DIR OR _PROBE_AGAIN)
        file(TO_CMAKE_PATH "$ENV{UASDK_BASE_DIR}" _TMP_BASE_DIR)
        if (NOT _PROBE_AGAIN AND _TMP_BASE_DIR)
            if (IS_DIRECTORY ${_TMP_BASE_DIR}/include/uastack)
                set(_BASE_DIR ${_TMP_BASE_DIR})
            else ()
                message(FATAL_ERROR "Environment variable UASDK_BASE_DIR does not point to folder containing AnsiC SDK include headers")
            endif ()
        ...
        elseif (NOT _PROBE_AGAIN AND IS_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}/../include/uastack)
            set(_BASE_DIR ${CMAKE_CURRENT_LIST_DIR}/..)
        endif ()

        if (NOT _BASE_DIR)
            message(FATAL_ERROR "Could not find folder containing AnsiC SDK include headers")
        else ()
            set(UASDK_BASE_DIR ${_BASE_DIR} CACHE PATH "Directory containing the UA SDK" FORCE)
            mark_as_advanced(UASDK_BASE_DIR)
        endif ()
    endif ()
    unset(_PROBE_AGAIN)
    set(_PREV_INSTALL_PREFIX ${CMAKE_INSTALL_PREFIX} CACHE INTERNAL "")
    ...
    get_option_from_file("${UASTACK_INCLUDE}/opcua_config.h" OPCUA_TRACE_ENABLE UASTACK_MINIMAL_DISABLE_TRACE)
    get_option_from_file("${UASTACK_INCLUDE}/opcua_config.h" OPCUA_ENCODEABLE_OBJECT_OMIT_TYPE_NAME UASTACK_MINIMAL_OMIT_TYPE_NAME)
    get_option_from_file("${UASTACK_INCLUDE}/opcua_config.h" OPCUA_DATAVALUE_OMIT_PICOSECONDS UASTACK_MINIMAL_SMALL_DATAVALUE)
    get_option_from_file("${UASTACK_INCLUDE}/opcua_config.h" OPCUA_VARIANT_OMIT_MATRIX UASTACK_MINIMAL_SMALL_VARIANT)
endif ()
