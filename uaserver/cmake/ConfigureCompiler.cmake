...
# disable_gcc_warning(warningname)
# warningname: e.g. "-Wpragmas"
...
# set_uac_level(uac_level)
# uac_level: one of "requireAdministrator", "highestAvailable" and "asInvoker"
macro (SET_UAC_LEVEL uac_level)
    if (MSVC)
        if (${MSVC_VERSION} GREATER 1500 AND NOT "${CMAKE_MAKE_PROGRAM}" MATCHES "ninja")
            set_target_properties(
                ${PROJECT_NAME}
                PROPERTIES LINK_FLAGS
                "/level='${uac_level}' /uiAccess='false' /SUBSYSTEM:WINDOWS"
            )
        else ()
            set_target_properties(
                ${PROJECT_NAME}
                PROPERTIES LINK_FLAGS
                "/MANIFESTUAC:\"level='${uac_level}' uiAccess='false'\" /SUBSYSTEM:WINDOWS"
            )
        endif ()
    endif ()
endmacro ()
...
if (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
    ...
    # This is usefull in automatic batch tests before release,
    # because release versions must compile warning free!
    option(ENABLE_GCC_TREAT_WARNINGS_AS_ERRORS "Set to ON to treat warnings as errors" OFF)
    if (ENABLE_GCC_TREAT_WARNINGS_AS_ERRORS)
    ...
    if (ENABLE_GCC_FORTIFY_SOURCE)
        if (NOT "${CMAKE_BUILD_TYPE}" OR "${CMAKE_BUILD_TYPE}" STREQUAL "Debug")
            ...
            # we cannot use CheckSymbolExists, as some compilers have _FORTIFY_SOURCE defined
            # internally, and that is not detected by CheckSymbolExists
            include(CheckCSourceCompiles)
            set(CMAKE_REQUIRED_DEFINITIONS "-D_FORTIFY_SOURCE")
            check_c_source_compiles("int main() {return 0;}" FORTIFY_SOURCE_NOT_DEFINED FAIL_REGEX "_FORTIFY_SOURCE")
            if (FORTIFY_SOURCE_NOT_DEFINED)
    ...
    if (CCACHE_APPLICATION)
        option(ENABLE_CCACHE_RULE "Prefix all compiler calls with 'ccache ', making use of ccache also for crosscompiling" ON)
...
if (MSVC)
    ...
    #   4204: nonstandard extension used : non-constant aggregate initializer (C99 standard)
    #   4251: 'identifier' : class 'type' needs to have dll-interface to be used by clients of class 'type2'
    #   4512: 'class' : assignment operator could not be generated
    #   4306: 'identifier' : conversion from 'type1' to 'type2' of greater size
    set(ADDITIONAL_C_WARNINGS "${ADDITIONAL_C_WARNINGS} /wd4127 /wd4200 /wd4201 /wd4204 /wd4251 /wd4512 /wd4306")
    ...
    # disable deprecated winsock API warnings for >= VS 2013
    ...
    # Check if Visual Leak Detector is installed and set HAVE_VISUAL_LEAK_DETECTOR accordingly
    include(CheckIncludeFile)
    check_include_file(vld.h VLD_EXISTS)
    cmake_dependent_option(ENABLE_VISUAL_LEAK_DETECTOR "Include vld.h for memory leak detection" OFF "VLD_EXISTS" OFF)
    if (ENABLE_VISUAL_LEAK_DETECTOR)
        add_definitions(-DHAVE_VISUAL_LEAK_DETECTOR)
    endif ()
endif ()
